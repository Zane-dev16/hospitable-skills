...
# Read verified 2026-09-22: dates OPTIONAL (missing end → start+14d; no params → today+14d, 200). Day: {date,day,min_stay,note,closed_for_checkin/closed_for_checkout,status:{reason,source,source_type,available},price:{amount,currency,formatted}}; money in minor units
...
- `calendar_restricted` channel flag → PUT fails 422 with channel message, so surface a friendly error instead of retrying blindly. (`calendar_restricted: false` confirmed top-level on this property.)
- Status `reason` observed: only `AVAILABLE`/`RESERVED` (`BLOCKED` never seen 2026-09-22); `note`/`closed_*` fields live but all null/false here. Reversed range → 400; >3y out → 400; single-day range → 200 with 1 day.
